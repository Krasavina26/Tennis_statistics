...
Выполнен на основе правил (rule-based).

## Деление кортов на зоны
*Уточнение: на данный момент не осуществлен никакой учет конкретной зоны корта.*

<table>
  <tr>
    <td align="center">
      <img width="600" height="303" alt="schema_before" src="https://github.com/user-attachments/assets/e7173268-a01e-4d0e-970a-fd1fad6fdddc" /><br>
      <em>Рисунок 1 Схема корта до деления сеткой на зоны</em>
    </td>
    <td align="center">
      <img width="600" height="303" alt="schema_after" src="https://github.com/user-attachments/assets/1f0abd97-f88f-48ec-8ce7-55ab7b32695e" /><br>
      <em>Рисунок 2 Схема корта после деления сеткой на зоны</em>
    </td>
  </tr>
</table>

Деление реального корта осуществляется следующим образом:
1) с помощью обученной модели детекции сетки находится bbox сетки на кадре;
2) нижняя линия bbox'а сетки определяется как линия сетки;
3) полученная линия сетки меняется так, чтобы стать параллельной другой линии корта (в этой реализации берется ближайшая нижняя горизонтальная линия, т.е. отрезок BC из Рисунка 3).
4) находятся пересечения линий зон, которые надо поделить сеткой, с линией сетки.

### Зачем делать линию сетки параллельной какой-либо другой линии?

В случаях наклона камеры или неровного ее изначального положения возможна следующая ситуация:
<div align="center">
  <img src="https://github.com/user-attachments/assets/ad45b513-e4eb-412a-a5bf-e67566d580c8" alt="корт_primer" />
  <br>
  <em>Рисунок 3 Пример определения линии сетки при наклоне камеры.</em>
</div>


Если не делать линию сетки параллельной, то линия сетки бы получилась HP вместо нужной AP.
